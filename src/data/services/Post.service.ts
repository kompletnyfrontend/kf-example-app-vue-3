...
export const getPost = async (id: Post["id"]): Promise<Post> => {
  try {
    ...
    requiredFields.forEach((field) => {
      ...
      // 👉 dalej mozemy sprawdzac, czy dane sa poprawnego typu, lub czy nie sa puste
      // 👉 mozemy tez sprawdzic, czy obiekty wewnatrz maja odpowiednie pola
    });
...
export const getPosts = async (): Promise<Posts> => {
  try {
    ...
    response.data.posts.forEach((post: UncheckedPost) => {
      requiredPostFields.forEach((field) => {
        if (!post[field]) throw new Error(`No ${field} field`);
      });
    });
